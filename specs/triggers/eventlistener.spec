...
Steps:
  ...
  * Verify trigger binding "pipeline-binding" exists
  * Verify trigger template "pipeline-template" exists
  * Verify event listener "listener-embed-binding" exists
...
Steps:
  ...
  * Verify trigger binding "gitlab-push-binding" exists
  * Verify trigger template "gitlab-echo-template" exists
  * Verify event listener "gitlab-listener" exists
...
Steps:
  ...
  * Verify trigger binding "bitbucket-binding" exists
  * Verify trigger template "bitbucket-template" exists
  * Verify event listener "bitbucket-listener" exists
...
Steps:
  ...
  * Verify trigger template "pipeline-template-git-push" exists
  * Verify event listener "listener-ctb-github-push" exists
...
Steps:
  ...
  * Verify trigger template "pipeline-template-git-pr" exists
  * Verify event listener "listener-clustertriggerbinding-github-pr" exists
  * Verify cluster trigger binding "github-pullreq" exists
  * Create & Link secret "github-secret" to service account "pipeline"
...
Steps:
  ...
  * Verify trigger template "pipeline-template-git-pr-review" exists
  * Verify event listener "listener-ctb-github-pr-review" exists
  * Verify cluster trigger binding "github-pullreq" exists
  * Create & Link secret "github-secret" to service account "pipeline"
...
Steps:
  ...
  * Verify trigger binding "github-pr-binding" exists
  * Verify trigger template "github-template" exists
  * Verify event listener "listener-triggerref" exists
  * Create & Link secret "github-secret" to service account "pipeline"
...
This scenario tests the creation of multiple event listeners with TLS enabled, listens to events forwards request to validator service -> parsed response to other validators and so on, on each event it creates/triggers
openshift-pipeline Resources defined under triggers-template, which helps you to deploy example app
